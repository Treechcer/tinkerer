...
function entitySpawner.func.spawn(dt)
    ...
    for i=0,spawns do
        ...
        if entities.isEntityOnTile(tileX, tileY, item.width, item.height) == -1 and renderer.checkCollsionWidthHeight(tileX, tileY, item.width, item.height) then
            entities.makeNewOne(tileX, tileY, item.entityName, item.HP, {item = item.drop.item, count = entitiesIndex.f.getCount(player.atributes[item.luck], item.drop.baseCount)}, item.width, item.height)
        end
